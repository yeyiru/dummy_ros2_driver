...
def get_dict(obj, is_config_object):
    ...
    for (k,v) in obj._remote_attributes.items():
        if isinstance(v, dummy_cli_tool.fibre.remote_object.RemoteProperty) and is_config_object:
            result[k] = v.get_value()
        elif isinstance(v, dummy_cli_tool.fibre.remote_object.RemoteObject):
...
def get_temp_config_filename(device):
    serial_number = dummy_cli_tool.fibre.utils.get_serial_number_str(device)
